...
// Listas circulares con nodo fantasma
...
std::ostream& operator<<(std::ostream& out, const Persona& persona) {
    out << persona.nombre; // Assuming getName() is a member function of Persona
    return out;
}
...
bool tratar_caso() {
    ...
    if (num_personas == 0 && edad_min == 0 && edad_max == 0) return false;
    ...
    lista.remove_if([&](const Persona& e){
        return (e.edad < edad_min || e.edad > edad_max);
