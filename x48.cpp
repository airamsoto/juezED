...
private:
    ...
    int count_interval(Node* node, const T &lower, const T &upper) const;
};
...
template <typename T>
int SetTree<T>::count_interval(SetTree::Node *node, const T &lower, const T &upper) const {
    if(node == nullptr) return 0;
    else {
        int contador = 0;
        if (node->elem >= lower && node->elem <= upper) contador++;
        if(node->elem < lower) {
            contador += count_interval(node->right, lower, upper);
        } else if (node->elem > upper) contador+= count_interval(node->left, lower, upper);
        else {
...
int SetTree<T>::count_interval(const T &lower, const T &upper) const {
    ...
    else return count_interval(root_node, lower, upper);
}
bool tratar_caso() {
    // Leemos los dos primeros números.
    int num_elems, num_consultas, aux, menor, mayor;
    SetTree<int> t;
    ...
    for (int i = 0; i < num_consultas; ++i) {
        cin >> menor;
        cin >> mayor;
        cout << t.count_interval(menor, mayor) << '\n';
    }
    cout << "---\n";
