// Perfil izquierdo de un árbol binario
// ------------------------------------
...
void calculaPerfilIzquierdo (const BinTree<int> & tree, int level, vector<int>& resultados) {
    if(!tree.empty()) {
        if(resultados.size() == level){
            resultados.push_back(tree.root());
            cout << tree.root() << " ";
        }
        calculaPerfilIzquierdo(tree.left(), level +1, resultados);
        calculaPerfilIzquierdo(tree.right(), level+1, resultados);
    }
...
vector<int> perfil(BinTree<int> arbol) {
    ...
    if(arbol.empty()) return resultados;
    else calculaPerfilIzquierdo(arbol, 0, resultados);
    return resultados;
}
