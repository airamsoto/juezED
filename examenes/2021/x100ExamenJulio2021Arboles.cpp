...
pair<bool, int> bienCod(const BinTree<int> &arbol) {
    if (arbol.empty()) {
        ...
        auto [bienIzq, cerosIzq] = bienCod(arbol.left());
        auto [bienDer, cerosDer] = bienCod(arbol.right());
        int totalCeros = cerosDer + cerosIzq;

        bool bien = true;
        if(abs(cerosIzq-cerosDer) > 1) bien = false;
        if(arbol.root() == 0) totalCeros++;


        return {bienDer && bienIzq && bien,totalCeros};
    }
}
